...
export function TagCombobox({ selectedIds, onChange, placeholder }: TagComboboxProps) {
  ...
  const showDropdown = isOpen && (filtered.length > 0 || showCreate || (inputValue.length > 0 && filtered.length === 0 && !showCreate))

  return (
    <div ref={containerRef} className="relative">
      {/* Input row */}
      <div
        className="flex flex-wrap items-center gap-1 border border-gray-300 rounded-lg px-3 py-2 min-h-[42px] cursor-text focus-within:ring-2 focus-within:ring-indigo-400 focus-within:border-transparent bg-white"
        onClick={() => {
          setIsOpen(true)
      ...
      >
        {selectedIds.map((id) => {
          ...
          return (
            <span
              key={id}
              className="inline-flex items-center gap-1 text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full shrink-0"
            >
              {tag.name}
              <button
                type="button"
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-md max-h-48 overflow-y-auto">
          ...
          {showCreate && (
            <div
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-indigo-600 hover:bg-indigo-50 cursor-pointer border-t border-gray-100"
            >
