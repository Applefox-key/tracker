...
function buildColumns(entries: Entry[]): { words: MatchCard[]; explanations: MatchCard[] } {
  return {
    words: shuffle(
      entries.map((e) => ({ id: `w-${e.id}`, entryId: e.id, type: 'word' as const, text: e.word })),
    ),
...
export function MatchPage() {
  ...
  // Rebuild columns when round changes
  useEffect(() => {
  ...
  // Auto-check when both sides selected
  useEffect(() => {
    ...
    if (selectedWord.entryId === selectedExplanation.entryId) {
      ...
      // Wrong pair — shake and clear after 600ms
  ...
  function advanceRound() {
    ...
    if (nextRoundStart >= allEntries.length) {
      setIsDone(true)
    } else {
      setRoundStart(nextRoundStart)
    }
  }
  ...
  function cardCls(
    ...
    const base =
      'w-full rounded-xl border px-3 py-2.5 text-left transition-all duration-150 select-none '

    if (isMatched) {
      return base + 'bg-green-50 border-green-300 text-green-700 cursor-default opacity-60'
    }
    if (isWrong) {
      return base + 'bg-red-50 border-red-400 text-red-700 cursor-default'
    }
    if (isSelected) {
      return base + 'bg-indigo-50 border-indigo-500 text-indigo-800 shadow-sm cursor-pointer'
    }
  ...
  const overallProgress =
    totalPairs > 0 ? Math.round(((totalMatched + matched.size) / totalPairs) * 100) : 0
            ...
            {explanationCards.map((card) => {
              ...
              return (
                <button
                  ...
                  disabled={matched.has(card.entryId) || wrongIds.size > 0}
                  className={[
                    cardCls(card, selectedExplanation?.id),
                    isWrong ? 'match-shake' : '',
                  ].join(' ')}
                >
                  <span
