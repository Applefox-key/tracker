...
interface Tile { id: string; value: string }
...
export function PuzzlePage() {
  ...
  useEffect(() => {
    ...
    setPool(tiles); setPlaced([]); setTileMode(mode); setAnswerPhase('thinking')
  ...
  function clearFilters() {
    setSelectedRatings([]); setSelectedCategory(null); setSelectedTag(null)
  }

  function startSession() {
    setQuestions(shuffle(filteredEntries)); setCurrentIdx(0); setScore(0); setIsDone(false); setIsPlaying(true)
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 flex flex-col items-center gap-6 text-center max-w-md mx-auto w-full">
          ...
          {filteredEntries.length === 0 && (
            <p className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">No matching entries</p>
          )}
  ...
  return (
    <div className="flex flex-col gap-6 max-w-xl mx-auto w-full">
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm text-gray-500 shrink-0 tabular-nums">{currentIdx + 1} / {questions.length}</span>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex flex-col gap-3">
        ...
        {currentEntry.example && (
          <p className="text-sm text-gray-500 italic border-l-2 border-indigo-200 pl-3">{currentEntry.example}</p>
        )}
      </div>

      <div className={['min-h-[56px] rounded-xl border-2 p-3 flex flex-wrap gap-2 items-center transition-colors',
        answerPhase === 'correct' ? 'border-green-400 bg-green-50'
        : answerPhase === 'wrong' ? 'border-red-400 bg-red-50'
        : 'border-dashed border-indigo-300 bg-indigo-50/40'].join(' ')}>
        {placed.length === 0 && answerPhase === 'thinking' && (
          <span className="text-sm text-indigo-300 italic">Click tiles below to place them here…</span>
        )}
        {placed.map((tile) => (
          <button key={tile.id} onClick={() => removePlaced(tile)} className="px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 active:bg-indigo-800 transition-colors">
            {tile.value}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {pool.map((tile) => (
          <button key={tile.id} onClick={() => placeTile(tile)} disabled={answerPhase !== 'thinking'} className="px-3 py-1.5 rounded-lg bg-white border border-gray-300 text-sm font-medium text-gray-700 hover:border-indigo-400 hover:bg-indigo-50 active:bg-indigo-100 transition-colors disabled:opacity-40">
            {tile.value}
          </button>
        ))}
      ...
      <p className="text-xs text-gray-400 text-center">{placed.length} / {tLen} tiles placed</p>
