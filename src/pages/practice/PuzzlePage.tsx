...
export function PuzzlePage() {
  ...
  // Load tiles when question changes
  useEffect(() => {
      ...
      >
        ...
        {placed.map((tile) => (
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 active:bg-indigo-800 transition-colors"
          >
            {tile.value}
          </button>
        ))}
        ...
        {answerPhase === 'wrong' && (
          <span className="ml-auto text-red-600 font-semibold text-sm">✗ Try again</span>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {pool.map((tile) => (
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-white border border-gray-300 text-sm font-medium text-gray-700 hover:border-indigo-400 hover:bg-indigo-50 active:bg-indigo-100 transition-colors disabled:opacity-40"
          >
            {tile.value}
          </button>
        ))}
      ...
      {answerPhase === 'wrong' && (
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleNext}>
            {currentIdx + 1 < questions.length ? 'Skip →' : 'Finish'}
          </Button>
        </div>
      )}
      {answerPhase === 'correct' && (
